...
def mock_ticker():
    ...
    def _factory(**overrides):
        stock = MagicMock()
        stock.income_stmt = _make_income_stmt(**overrides)
        stock.balance_sheet = _make_balance_sheet(**overrides)
        stock.cashflow = _make_cashflow(**overrides)
        stock.info = {
...
class TestCalculateROIC:
    ...
    def test_zero_pretax_returns_none(self, mock_ticker):
        stock = mock_ticker(pretax_income=0)
        result = calculate_roic(stock)
    ...
    def test_negative_invested_capital_returns_none(self, mock_ticker):
        ...
        stock = mock_ticker(total_assets=100, current_liabilities=200, cash=50)
        result = calculate_roic(stock)
        assert result is None

    def test_key_error_returns_none(self):
        stock = MagicMock()
        stock.income_stmt = pd.DataFrame()  # empty — will raise KeyError
...
class TestCalculateFCFConversion:
    def test_normal_case(self, mock_ticker):
        stock = mock_ticker(fcf=120_000, net_income=100_000)
        result = calculate_fcf_conversion(stock)
        assert result is not None
        assert pytest.approx(result, rel=0.01) == 1.2

    def test_negative_net_income_returns_none(self, mock_ticker):
        stock = mock_ticker(net_income=-10_000)
        result = calculate_fcf_conversion(stock)
        assert result is None
...
class TestCalculateNetDebtEBITDA:
    def test_normal_case(self, mock_ticker):
        stock = mock_ticker(total_debt=80_000, cash=50_000, ebitda=150_000)
        result = calculate_net_debt_ebitda(stock)
        assert result is not None
        # (80k - 50k) / 150k = 0.2
        assert pytest.approx(result, rel=0.01) == 0.2

    def test_net_cash_returns_negative(self, mock_ticker):
        stock = mock_ticker(total_debt=10_000, cash=50_000, ebitda=150_000)
        result = calculate_net_debt_ebitda(stock)
        assert result is not None
        assert result < 0
...
class TestCalculatePriceToBk:
    def test_returns_value(self, mock_ticker):
        stock = mock_ticker(pb=3.5)
        result = calculate_price_to_book(stock)
        assert result == pytest.approx(3.5, rel=0.01)


class TestCalculateCurrentRatio:
    def test_normal_case(self, mock_ticker):
        stock = mock_ticker(current_assets=200_000, current_liabilities=100_000)
        result = calculate_current_ratio(stock)
        assert result == pytest.approx(2.0, rel=0.01)


class TestCalculateTrailingEarningsGrowth:
    def test_returns_growth(self, mock_ticker):
        stock = mock_ticker(earnings_growth=0.15)
        result = calculate_trailing_earnings_growth(stock)
...
class TestBuildFortressMetrics:
    def test_returns_metrics_list(self, mock_ticker):
        stock = mock_ticker()
        metrics = build_fortress_metrics(stock)
...
class TestBuildEngineMetrics:
    def test_returns_metrics_with_gross_margin(self, mock_ticker):
        stock = mock_ticker(gross_margins=0.40)
        metrics = build_engine_metrics(stock)
        names = {m.metric_name for m in metrics}
...
class TestBuildAlignmentMetrics:
    def test_returns_insider_ownership(self, mock_ticker):
        stock = mock_ticker(insider_pct=0.10)
        metrics = build_alignment_metrics(stock)
        names = {m.metric_name for m in metrics}
